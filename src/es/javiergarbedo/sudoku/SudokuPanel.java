        ...
        if (sudoku != null && !Beans.isDesignTime()) {
            ...
            for (int i = 0; i <= NUM_COLUMNS; i++) {
                for (int j = 0; j <= NUM_ROWS; j++) {
                    if (sudokuPanel.getRowSelected() != -1 && sudokuPanel.getColSelected() != -1) {
                        Point rectanglePoint = new Point(i * CELL_SIZE, j * CELL_SIZE);
                        g.setColor(BACKGROUND_SELECTED);
                        g.fillRect(rectanglePoint.x, rectanglePoint.y, CELL_SIZE, CELL_SIZE);
                    } else {
                        if (sudoku.isInInitialPuzzle(sudokuPanel.getRowSelected(), sudokuPanel.getColSelected())) {
                            Point rectanglePoint = new Point(i * CELL_SIZE, j * CELL_SIZE);
            ...
            for(int i = 0 ; i<NUM_ROWS; i++){
                for(int j = 0; j<NUM_COLUMNS; j++){
                    ...
                    g.setFont(myFont);
                 g.drawString(String.valueOf(sudoku.getValue(i,j)),CELL_SIZE * i,CELL_SIZE * j);
                }
            }
